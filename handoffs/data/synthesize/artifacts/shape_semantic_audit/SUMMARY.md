...
因此，GPU reference 和 changed-region 通过不能证明 shape 有任务语义。当前候选池仍应保持 `training_approved=false`。1/264 只是在启发式极端、分层覆盖样本中的已确认 child 新增问题命中率，不能外推为 63,979 条的总体比例。

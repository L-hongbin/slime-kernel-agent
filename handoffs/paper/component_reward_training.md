...
当前用一套通用源码规则识别优化实现，再将最佳答案中的具体实现追溯到最早出现的轮次。规则库包含 9 个类别，58 项模板。训练先计算原 TRLOO 累计回报，再从通过 correctness 且 speedup ≥ 1 的答案中选择最佳答案，对提供早期组件的轮次加入系数为 0.25 的来源份额。最终累计回报不封顶，最佳轮与其它轮保留原回报
...
签名保留源码中的名字、类型、常数、运算顺序和调用参数。函数正文按去除空白与注释后的 token 记录，launch 配置和实参还保留源码表达式。组件描述另外保存从 `ModelNew.forward` 出发的静态调用路径和 profiling 命中名称，供下一步判断最佳答案是否使用了该组件
...
因此，三个标签共同描述 A，A 仍只占一份 credit。若将 block 大小从 `128` 改成 `256` 并调整 grid 表达式，即使 kernel 正文未变，整体实现签名也会变化；本例仅在代码末尾增加注释时，签名保持不变。这两种变化均已用当前分析器核对
...
分配顺序是先按最佳答案的组件等分，再按来源轮次汇总。这里得到的 $C_t$ 是来源份额，尚不是最终训练目标。先保留通过 correctness 且有有限实测 speedup ≥ 1 的合法答案，再按原 task reward 选择其中最高质量轮，平分时取最早一轮；来源只在原 baseline 保留的入训轮次中搜索。记最佳轮为 $t^\star$，其正质量预算为 $V = \max(q_{t^\star}, 0)$，组件描述中的全部分奖单位数为 $K$
...
因此，来源份额按各自提供的最佳答案组件数量分配。A、B 在第三轮重新出现，它们的份额仍全部归第一轮；F 未进入最佳答案，因此不占份额。最佳轮的来源份额可以为零，但其最终训练目标始终保留原 TRLOO 回报。最佳轮自己的份额和无法解析的 residual 只作归因记录，不另外加奖
...
先完整执行 baseline 的 finalize 与回报累加。记第 $t$ 轮的原 TRLOO 累计回报为 $G_t$；当前折扣为 1，被 baseline 移除轮次的本轮贡献记零

$$
G_t=\sum_{u=t}^{T}q_u
$$

从被 baseline 保留、通过 correctness、实测 speedup 有限且不小于 1 的轮次中，按原质量分选择最佳轮 $t^\star$，同分时选较早轮。选择范围必须先经过门槛筛选，避免一个 coverage 更高但较慢的答案挡住已有的加速答案。没有合格答案时，额外项全部为零
...
$$
Y_t=\begin{cases}G_t+0.25C_t,&t\in S\\G_t,&t\notin S\end{cases}
$$

来源轮必须已经被 baseline 保留，但本身可以失败或较慢；它的资格由后续合格答案是否保留该实现决定。最佳轮、最佳轮之后，以及没有获得早期来源份额的轮次，完全沿用原回报。所有组件仍共同分享一个质量预算 $V$，每份只归属一次，因此总额外项不超过 $0.25V$；这是来源份额归一化的结果，不对原累计回报做裁剪

沿用 A/B/C/D 例子，假设前两轮失败、第三轮正确且通过 speedup 门槛，原任务分为 $[0,0,1.20]$，因此原累计回报为 $[1.20,1.20,1.20]$

| 轮次 | 来源份额 $C_t$ | 原回报 $G_t$ | 额外项 | 最终目标 $Y_t$ |
|---|---:|---:|---:|---:|
| 第一轮，A/B 来源 | 0.60 | 1.20 | 0.15 | 1.35 |
| 第二轮，C 来源 | 0.30 | 1.20 | 0.08 | 1.28 |
| 第三轮，正确最佳 | 0.30 | 1.20 | 0.00 | 1.20 |

第一轮额外获得 0.15，第二轮额外获得 0.075。第三轮的 D 不产生额外奖励，第三轮仍保留自己的原回报 1.20。表格保留两位小数，实际计算使用未取整值

$Y_t$ 直接进入同题、同轮次的 TRLOO leave-one-out 比较，不再向前累加一次。来源份额和最终 advantage 是两回事：某轮目标为正，仍可能低于其它轨迹同轮的目标而获得负 advantage。packing、训练 mask 和 MTP 的有效 token 集合沿用 baseline

采样过滤沿用原 baseline 的末轮 task reward 规则，不使用加奖后的 $Y_t$。原 reward、累计回报、来源份额、额外项、门槛资格及最终目标分别记录。部分分及其原 TRLOO 回传规则也保持不变
...
- **最佳轮由合格答案中的现有质量分决定**：先要求正确且 speedup ≥ 1，再沿用 baseline 的 task reward 排序；其中存在上限或并列，所以选中的最佳轮不一定是 raw speedup 最大的一轮

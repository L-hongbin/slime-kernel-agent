...
这套细粒度访存取证仍有明显成本。两份 FP16 样本的附加诊断各约 19 秒，而插桩 forward 仅约 0.89–1.06 秒；RNN 的附加诊断约 23 秒。不能把 forward 的一秒当作每题总开销，后续要用新增的逐阶段 wall time 分清进程启动、快照重放和 CPU 整图成本，完整轨迹池覆盖率与吞吐尚未验收

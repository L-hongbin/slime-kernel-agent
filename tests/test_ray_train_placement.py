...
def test_rollout_port_allocation_uses_each_engine_actual_host(monkeypatch):
    ...
    from slime.backends.sglang_utils import engine_group as engine_group_module
    ...
    monkeypatch.setattr(engine_group_module.ray, "get", lambda value: value)

    allocated, cursors = engine_group_module._allocate_rollout_engine_addr_and_ports_normal(
        args=args,
...
def test_rollout_port_allocation_fails_if_actor_moves_hosts(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="moved nodes during port allocation"):
        engine_group_module._allocate_rollout_engine_addr_and_ports_normal(
            args=args,
...
def test_rollout_actor_disables_sglang_tp_memory_imbalance_guard():
    source = Path(__file__).resolve().parents[1] / "slime" / "backends" / "sglang_utils" / "engine_group.py"

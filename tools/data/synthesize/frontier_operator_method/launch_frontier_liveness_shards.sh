#!/usr/bin/env bash
# Launch the source-bound exact-1k frontier liveness proof on eight idle GPUs.
set -euo pipefail
...
trials=${FRONTIER_LIVENESS_TRIALS:-3}
seed=${FRONTIER_LIVENESS_SEED:-17}
timeout_seconds=${FRONTIER_LIVENESS_TIMEOUT_SECONDS:-600}
idle_memory_mib=${FRONTIER_IDLE_MEMORY_MIB:-128}
validator_path=tools/data/synthesize/frontier_operator_method/validate_frontier_liveness.py
generator_path=tools/data/synthesize/frontier_operator_method/generate_frontier_operator.py
semantic_validator_path=tools/data/synthesize/semantic_operator_method/validate_semantic_liveness.py
semantic_validator_sha256=b0177d19377f71ae32aac13698079f858942ad9f95f866a2192bfa50f933b7ff
...
[[ -f ${validator_path} && -f ${generator_path} && -f ${semantic_validator_path} ]] || die "frontier source closure missing"
[[ ${trials} =~ ^[0-9]+$ ]] && (( trials == 3 )) || die "FRONTIER_LIVENESS_TRIALS must be exactly 3"
[[ ${seed} == 17 ]] || die "FRONTIER_LIVENESS_SEED is fixed to 17"
[[ ${idle_memory_mib} =~ ^[0-9]+$ ]] || die "FRONTIER_IDLE_MEMORY_MIB must be non-negative"

# A typo must never turn an empty old lane into a frontier result directory.
run_dir_real=$(realpath -m "${run_dir}")
case ${run_dir_real} in *frontier_operator*) ;; *) die "RUN_DIR must be an explicit frontier_operator lane: ${run_dir_real}" ;; esac
case ${run_dir_real} in
  *semantic_operator*|*random_method*|*dtype_method*|*layout_method*) die "RUN_DIR points at a different synthesis lane: ${run_dir_real}" ;;
esac
...
semantic_observed_sha256=$(sha256sum "${semantic_validator_path}" | awk '{print $1}')
candidates_sha256=$(sha256sum "${candidates_path}" | awk '{print $1}')
manifest_sha256=$(sha256sum "${manifest_path}" | awk '{print $1}')
allowlist_sha256=$(sha256sum "${allowlist_path}" | awk '{print $1}')
[[ ${semantic_observed_sha256} == "${semantic_validator_sha256}" ]] || die "semantic validator SHA binding mismatch"

# NFS worktrees can retain bytecode from an immediately preceding source edit.
# A fresh prefix forces every launcher/worker import to compile the hash-bound
# sources above instead of consulting a repository-local __pycache__.
python_cache_root=$(mktemp -d /tmp/prompt_tvm_v4_frontier_pycache.XXXXXX)
export PYTHONPYCACHEPREFIX=${python_cache_root}

python3 - "${candidates_path}" "${manifest_path}" "${allowlist_path}" "${generator_sha256}" "${gpu_count}" "${max_authorized_candidates}" <<'PY'
from __future__ import annotations
import hashlib, json, sys
from pathlib import Path
import pyarrow.parquet as pq
candidates, manifest, allowlist, generator_sha, gpu_count, max_authorized_candidates = Path(sys.argv[1]), Path(sys.argv[2]), Path(sys.argv[3]), sys.argv[4], int(sys.argv[5]), int(sys.argv[6])
...
if len(rows) != max_authorized_candidates or len(manifests) != max_authorized_candidates:
    raise SystemExit(f"frontier canary must contain exactly {max_authorized_candidates} aligned rows:{len(rows)}:{len(manifests)}")
...
for index, (row, item) in enumerate(zip(rows, manifests, strict=True)):
    ...
    if item.get("primary_intervention") != "frontier_operator_scenario":
        raise SystemExit(f"frontier intervention mismatch:{index}")
    if item.get("parent_uuid") is not None or item.get("lineage_kind") != "standalone_frontier_synthetic":
        raise SystemExit(f"frontier row is not parentless:{index}")
    ...
    if item.get("final_output_contract") != {"kind":"single_dense_tensor","finite_required":True}:
...
if len(allowed) > max_authorized_candidates or len(allowed) < gpu_count:
    raise SystemExit(f"allowlist count must be in [{gpu_count},{max_authorized_candidates}]:{len(allowed)}")
...
flock -n "${run_lock_fd}" || die "another frontier launcher owns ${run_dir}"
exec {global_lock_fd}>/tmp/prompt_tvm_v4_frontier_reference_validation_all_visible_gpus.lock
flock -n "${global_lock_fd}" || die "another frontier validation launcher owns the visible GPU set"

for source_path in "$0" "${validator_path}" "${generator_path}" "${semantic_validator_path}"; do
  case ${source_path} in
    ...
    *) archive_path=${run_dir}/semantic_validator_source.py ;;
  esac
...
python3 - "${scheduler_tmp}" "${launcher_sha256}" "${validator_sha256}" "${generator_sha256}" "${semantic_observed_sha256}" \
...
# Initialize every physical device serially before the eight validator parents
# begin spawning row subprocesses.  Concurrent first-use CUDA initialization
# has produced intermittent ``CUDA device required`` diagnostics on this host;
# the preflight is a measurement guard, not a row-level retry.
...
(( status == 0 )) || die "one or more frontier-liveness shards had an infrastructure failure; raw shard records were retained"

python3 - "${run_dir}" "${gpu_count}" "${launcher_sha256}" "${validator_sha256}" "${semantic_observed_sha256}" "${allowlist_path}" <<'PY'
from __future__ import annotations
import json, sys
from pathlib import Path
run, count, launcher, validator, semantic, allowlist = Path(sys.argv[1]), int(sys.argv[2]), sys.argv[3], sys.argv[4], sys.argv[5], Path(sys.argv[6])
summaries=[]
for index in range(count):
    ...
    if record.get("semantic_validator_source_sha256") != semantic:
        raise SystemExit(f"semantic source binding mismatch:{path}")
    summaries.append(record)
bindings={record.get("contract_binding_sha256") for record in summaries}
...
payload={
    ...
    "resumed":sum(item["resumed"] for item in summaries), "passed":sum(item["passed"] for item in summaries),
    "failed":sum(item["failed"] for item in summaries), "shards":summaries,
}
allowed=[line.strip() for line in allowlist.read_text(encoding="utf-8").splitlines() if line.strip()]

...
__global__ void broadcast_repeat(const volatile float* a,float* b){int i=threadIdx.x;if(i<16){float x=0;for(int j=0;j<10000;++j)x+=a[0];b[i]=x;}}
__global__ void mixed_conflict(float* b){int i=threadIdx.x;b[0]=float(i);if(i==0){b[4]=1.0f;b[512]=2.0f;}}
__global__ void sparse_copy(const float* a,float* b){int i=threadIdx.x;if(i<8)b[i*2]=a[i*2];}
...
int main(int argc,char**argv){int mode=argc>1?atoi(argv[1]):0;const int n=mode==14?1024:16;float a[n],out[n];for(int i=0;i<n;++i)a[i]=float(i);float *input,*tmp,*output;CUDA(cudaMalloc(&input,sizeof a));CUDA(cudaMalloc(&tmp,sizeof a));CUDA(cudaMalloc(&output,sizeof a));CUDA(cudaMemcpy(input,a,sizeof a,cudaMemcpyHostToDevice));
 ...
 CUDA(cudaMemset(output,0,sizeof out));
 auto enable=reinterpret_cast<void(*)(int)>(dlsym(RTLD_DEFAULT,"runtime_trace_set_enabled"));if(enable)enable(1);
 ...
 else if(mode==15){add_one<<<1,32>>>(input,tmp,n);add_one<<<1,32>>>(tmp,tmp,n);add_one<<<1,32>>>(tmp,output,n);}
 else if(mode==14) mixed_conflict<<<1,32>>>(output);
 else if(mode==12) broadcast_repeat<<<1,32>>>(input,output);
 else if(mode==13){CUDA(cudaMemset(output,0,sizeof a));sparse_copy<<<1,32>>>(input,output);}
 ...
 CUDA(cudaDeviceSynchronize());if(enable)enable(0);CUDA(cudaMemcpy(out,output,sizeof out,cudaMemcpyDeviceToHost));printf("{\"mode\":%d,\"output\":[",mode);for(int i=0;i<n;++i)printf("%s%.9g",i?",":"",out[i]);printf("]}\n");CUDA(cudaFree(input));CUDA(cudaFree(tmp));CUDA(cudaFree(output));return 0;}

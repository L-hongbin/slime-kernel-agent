...
def _accumulate_main_grad(total_input: torch.Tensor, grad_output: torch.Tensor, weight: torch.Tensor) -> None:
    ...
    if main_grad is None:
        raise RuntimeError("--fp32-lm-head requires weight.main_grad for gradient accumulation fusion.")
...
def _fp32_lm_head_forward(
    ...
    if output_cross_entropy_loss:
        raise NotImplementedError("--fp32-lm-head requires cross_entropy_loss_fusion=False.")
...
def enable_fp32_lm_head(model: torch.nn.Module) -> None:
    ...
    if output_layer is None:
        raise RuntimeError("--fp32-lm-head was requested on a post-process model without output_layer.")

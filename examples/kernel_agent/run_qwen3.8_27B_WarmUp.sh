...
cd "${REPO_ROOT}"
# NODE CONFIG
MASTER_ADDR="192.168.112.2"
case "${MASTER_ADDR}" in
    192.168.112.2)
      ACTOR_NUM_NODES=4
      CP_SIZE=2
      REMOTE_HOSTS=(
        "192.168.112.73" "192.168.112.7" "192.168.112.57" "192.168.112.54" "192.168.112.70"
      )
      REMOTE_PORTS=(
        17699 17699 17699 17699 17699
      )
      ;;
    *)
...
KERNEL_ENV_URL="${KERNEL_ENV_URL:-"http://192.168.116.33:20111"}"
...
DATASET="${DATASET:-"TorchOpsV4_DifficultyLt18"}"
...
export CUDA_AGENT_RETURN_DETAIL_CORRECTNESS="${CUDA_AGENT_RETURN_DETAIL_CORRECTNESS:-0}"
export CUDA_AGENT_RETURN_DETAIL_COMPILATION="${CUDA_AGENT_RETURN_DETAIL_COMPILATION:-0}"
...
case "${DATASET}" in
    "TorchOpsV4_DifficultyLt18")
      ...
      # 外网："/nfs/FM/lihongbin/datasets/KernelData/prompt_tvm_GEPA4o_v2/torch_ops_difficulty_lt18.parquet"
      ;;
    *)
...
EXP_ARGS+=("${MIS_ARGS[@]}")
EXP_NAME="Kernel-FAsyncWarmUp-${KERNEL_BACKEND^^}"
...
RUNTIME_ENV_JSON=$(cat <<EOF_JSON
{
  "env_vars": {
    ...
    "CUDA_AGENT_RETURN_DETAIL_COMPILATION": "${CUDA_AGENT_RETURN_DETAIL_COMPILATION}",

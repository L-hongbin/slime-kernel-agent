...
EXP_NAME="qwen3_14B_cuda_rl_fully_async"
...
# Fully-async uses disjoint actor and rollout GPUs; train_async.py does not support --colocate.
GPUS_PER_NODE=${GPUS_PER_NODE:-2}
ACTOR_NUM_GPUS_PER_NODE=${ACTOR_NUM_GPUS_PER_NODE:-1}
ROLLOUT_NUM_GPUS=${ROLLOUT_NUM_GPUS:-$((NUM_NODES * GPUS_PER_NODE - NUM_NODES * ACTOR_NUM_GPUS_PER_NODE))}
...
if [ "${ROLLOUT_NUM_GPUS}" -le 0 ]; then
   echo "ROLLOUT_NUM_GPUS must be > 0 for fully-async training. Set GPUS_PER_NODE/ACTOR_NUM_GPUS_PER_NODE/ROLLOUT_NUM_GPUS."
   exit 1
fi

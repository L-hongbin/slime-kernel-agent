...
SAVE_DEBUG_ROLLOUT_KEEP_LATEST=${SAVE_DEBUG_ROLLOUT_KEEP_LATEST:-0}
if [[ "${SAVE_DEBUG_ROLLOUT_KEEP_LATEST}" != "0" && "${SAVE_DEBUG_ROLLOUT_KEEP_LATEST}" != "1" ]]; then
   echo "SAVE_DEBUG_ROLLOUT_KEEP_LATEST must be 0 or 1." >&2
   exit 1
fi
...
SGLANG_LOGPROB_CHUNK_SIZE=${SGLANG_LOGPROB_CHUNK_SIZE:-256}
if ! [[ "${SGLANG_LOGPROB_CHUNK_SIZE}" =~ ^[1-9][0-9]*$ ]]; then
   echo "SGLANG_LOGPROB_CHUNK_SIZE must be a positive integer." >&2
   exit 1
fi
...
case "${SGLANG_SERVING_PROFILE}" in
   matched)
      # Long in-flight responses become input-logprob prefill after refit.
      # Leave room for full-vocabulary logits and their FP32 softmax buffers.
...
if [[ "${CONFIG_DRY_RUN}" == "1" ]]; then
   ...
   printf 'SGLANG_ENABLE_LOGPROB_CHUNK=1\nSGLANG_LOGPROB_CHUNK_SIZE=%s\n' "${SGLANG_LOGPROB_CHUNK_SIZE}"
...
RUNTIME_ENV_JSON=$(cat <<EOF_JSON
{
  "env_vars": {
    ...
    "SGLANG_ENABLE_LOGPROB_CHUNK": "1",
    "SGLANG_LOGPROB_CHUNK_SIZE": "${SGLANG_LOGPROB_CHUNK_SIZE}",
    "SLIME_LOG_TRAIN_MICROBATCH_SHAPES": "1",
    "SLIME_SAVE_DEBUG_ROLLOUT_MAX_ID": "${SAVE_DEBUG_ROLLOUT_MAX_ID}",
    "SLIME_SAVE_DEBUG_ROLLOUT_KEEP_LATEST": "${SAVE_DEBUG_ROLLOUT_KEEP_LATEST}",

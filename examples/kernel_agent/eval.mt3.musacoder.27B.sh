...
export no_proxy="127.0.0.1,localhost,0.0.0.0,::1,${MASTER_ADDR:-192.168.16.22},192.168.16.21"
...
EVAL_DATA=${EVAL_DATA:-/nfs/FM/chenshuailin/projects/kernel_agents/slime-dev-csl-2/Data/kernelbench-level1-validation-musa-coder-load-inline-oldsize-rand/train.parquet}
KERNEL_ENV_URL=${KERNEL_ENV_URL:-http://192.168.16.21:20111}
KERNEL_BACKEND=${KERNEL_BACKEND:-load_inline}
...
# Align with MusaCoder RL: max prompt 8K + max response 32K = 40K total context.
...
EXP_NAME="EvalMT${MAX_TURNS}.${KERNEL_BACKEND}.MusaCoder-27B.CTX${MAX_CONTEXT_LEN}"
...
CUSTOM_ARGS=(
   ...
   --multi-turn-prompt-config-path "${SCRIPT_DIR}/prompt_config/multi_turn_load_inline.yaml"
)
...
KERNEL_AGENT_ARGS=(
   ...
   --use-reference-cache
   --use-multi-turn
   # Keep every generated turn for eval; otherwise the parser default is
   # finalize-mode=positive (only the first correct turn is retained).
   --finalize-mode none
   --max-turns ${MAX_TURNS}
)
...
RUNTIME_ENV_JSON=$(cat <<EOF_JSON
{
  "env_vars": {
    ...
    "CUDA_AGENT_LOG_MULTI_TURN_TEXT": "${CUDA_AGENT_LOG_MULTI_TURN_TEXT:-0}",
    "KERNEL_AGENT_GENERATE_GUARD_SEC": "${KERNEL_AGENT_GENERATE_GUARD_SEC:-0}",
...
ray job submit --address="http://${MASTER_ADDR}:${RAY_DASHBOARD_PORT}" \
   ...
   -- python3 train.py \
   --actor-num-gpus-per-node ${ACTOR_NUM_GPUS:-8} \
   --colocate \
...
python3 "${REPO_ROOT}/examples/kernel_agent/summarize_eval.py" "${EVAL_DIR}" --max-turns ${MAX_TURNS} | tee "${SUMMARY_PATH}"

...
offset，不改写 `sample.reward`、`task_reward`、组件或 verify 历史数据。偏移后仍为负或非有限值时报错；
不会按组减最小值或将负数截成零。
...
Verify 当前仅支持 `group` baseline，不能同时使用 history/anchor/greedy-anchor。
...
`--normalize-advantages`，不支持 verify history/anchor baseline；其它 loss/过滤/CTM/OPD 设置保持独立。

...
DEFAULT_KERNELGYM_URL = "http://127.0.0.1:20391"
...
def _record_columns(records: list[dict[str, Any]]) -> list[str]:
    ...
    keys = {
        ...
        if _is_scalar(value) and key not in hidden
    }
...
class KernelGymHealthClient:
    ...
    async def check_health(self) -> dict[str, Any]:
        ...
        if status != "healthy":
            raise KernelGymRequestError(
                f"KernelGym at {self.base_url} is not healthy (status={status!r}): {payload}"
            )
        return payload
